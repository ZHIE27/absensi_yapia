...
const StaffTable = () => {
  ...
  const exportPDF = () => {
    ...
    doc.save('data_absensi_staff.pdf');
  };

  const exportExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(staffData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'DataAbsensiStaff');
    XLSX.writeFile(workbook, 'data_absensi_staff.xlsx');
  };

  return (
    <div className="p-3">
      <div className="p-6 bg-yellow-200 border-[3px] border-black shadow-[6px_6px_0_rgba(0,0,0,1)]">
        <h1 className="text-2xl font-extrabold mb-4 text-black">📋 Data Absensi Staff</h1>
